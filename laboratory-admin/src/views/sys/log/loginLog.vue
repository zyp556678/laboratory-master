<template>
  <div>
    ...
    <el-row>
      <el-form :inline="true">
        <el-form-item>
          <el-input v-model="searchObj.username" placeholder="登录用户" />
        </el-form-item>
        <el-form-item>
          <el-input v-model="searchObj.loginAddr" placeholder="登录地址" />
        </el-form-item>
        <el-form-item>
          <el-select v-model="searchObj.loginState" placeholder="登录状态" @change="getAll">
    ...
    <el-row>
      ...
      >
        ...
        <el-table-column prop="username" fixed="left" label="登录用户" show-overflow-tooltip sortable />
        <el-table-column prop="loginIp" label="登录IP" show-overflow-tooltip sortable />
        <el-table-column prop="loginAddr" label="登录地址" show-overflow-tooltip />
        <el-table-column prop="driverName" label="浏览器" show-overflow-tooltip />
        <el-table-column prop="osName" label="操作系统" show-overflow-tooltip />
        <el-table-column prop="loginState" label="登录状态" show-overflow-tooltip sortable>
          <template slot-scope="scope">
            <el-tag v-if="scope.row.loginState === 1" type="success" effect="plain">成功</el-tag>
            <el-tag v-if="scope.row.loginState === 0" type="danger" effect="plain">失败</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="loginTime" label="登录时间" show-overflow-tooltip sortable />
        <el-table-column prop="loginResult" label="登录结果" show-overflow-tooltip />
      </el-table>
    </el-row>
